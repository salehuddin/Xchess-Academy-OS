...
export default function StudentDetailsModal({ token, isOpen, onClose, student: initialStudent }) {
    ...
    const fetchStudentDetails = async (id) => {
        setLoading(true);
        try {
            const response = await axios.get(route('portal.students.details', [token, id]));
                                                ...
                                                {student.classes.map((cls) => (
                                                    <div key={cls.id} className="flex justify-between items-center p-3 border rounded-lg hover:bg-default-50 transition-colors">
                                                        <div className="flex flex-col gap-1">
                                                            <span className="font-bold text-small">{cls.name}</span>
